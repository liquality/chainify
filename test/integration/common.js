...
async function deployERC20Token (chain) {
  before(async () => {
    ...
    const originalSendTransaction = ethereumRpcProvider.sendTransaction
    ethereumRpcProvider.sendTransaction = async (to, value, data) => {
      const txHash = await originalSendTransaction.bind(ethereumRpcProvider)(to, value, data)
      if (data !== null) {
        await mineBlock(chain)
      }
      return txHash
    ...
    let txHash = await ethereumRpcProvider.sendTransaction(null, 0, bytecode)
    await mineBlock(chain)
