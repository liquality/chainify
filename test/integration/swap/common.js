...
function deployERC20Token (client) {
  before(async () => {
    console.log('\x1b[36m', 'Deploy the ERC20 token', '\x1b[0m')
    ...
    await client.getMethod('sendTransaction')(null, 0, bytecode)
    let blockNumber = await client.getMethod('getBlockHeight')()
    let tokenDeployedTransaction = null
    while (!tokenDeployedTransaction) {
      const block = await client.getMethod('getBlockByNumber')(blockNumber, true)
      if (block) {
        tokenDeployedTransaction = block.transactions.find(transaction =>
          transaction.input === bytecode
        )
        blockNumber++
      }
      await sleep(5000)
    }
    const initiationTransactionReceipt = await client.getMethod('getTransactionReceipt')(tokenDeployedTransaction.hash)
    client.getMethod('setErc20')(initiationTransactionReceipt.contractAddress)
  })
}
