...
function testSwap(chain: Chain) {
  ...
  xit('Claim only works using correct secret', async () => {
  ...
  xit('Claim fails with secret longer than 32 bytes', async () => {
  ...
  xit('Claim fails with secret shorter than 32 bytes', async () => {
  ...
  if (chain.name !== 'near' && chain.name !== 'solana' && chain.name !== 'flow') {
    ...
    xit('Refund available after expiration', async () => {
      const secretHash = crypto.sha256(mockSecret)
      const swapParams = await getSwapParams(chain, secretHash)
...
// function testSolanaRefund(chain: Chain) {
//   it('Refund fails after claim', async () => {
...
//             await refundAndVerify(chain, initiationTxId, swapParams)
//           } catch (e) {
//             expect(e.message).equal('AccountDoesNotExist')
//             fee = new BigNumber(0)
