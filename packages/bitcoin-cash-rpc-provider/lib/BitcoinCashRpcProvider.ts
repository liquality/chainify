...
export default class BitcoinCashRpcProvider extends JsonRpcProvider implements Partial<ChainProvider> {
  ...
  constructor(options: ProviderOptions) {
    const { uri, username, password, network, feeBlockConfirmations = 1, defaultFeePerByte = 1 } = options
  ...
  async decodeRawTransaction(rawTransaction: string): Promise<bitcoinCash.Transaction> {
  ...
  async getUnspentTransactions(_addresses: (Address | string)[]): Promise<bitcoinCash.UTXO[]> {
    const addresses = _addresses.map(addressToString)
    const utxos: bitcoinCash.rpc.UTXO[] = await this.jsonrpc('listunspent', 0, 9999999, addresses)
  ...
  async getAddressTransactionCounts(_addresses: (Address | string)[]) {
    const addresses = _addresses.map(addressToString)
    const receivedAddresses: bitcoinCash.rpc.ReceivedByAddress[] = await this.jsonrpc(
      'listreceivedbyaddress',
      0,
      false,
      true
    )
    return addresses.reduce((acc: bitcoinCash.AddressTxCounts, addr) => {
  ...
  async getBlockByHash(blockHash: string, includeTx = false): Promise<Block> {
    let data: bitcoinCash.rpc.Block
  ...
  async getTransactionFee(tx: bitcoinCash.Transaction) {
  ...
  async getParsedTransactionByHash(
    ...
    addFees = false
  ): Promise<Transaction<bitcoinCash.Transaction>> {
    const tx: bitcoinCash.rpc.MinedTransaction = await this.jsonrpc('getrawtransaction', transactionHash, 1)
  ...
  async fundRawTransaction(hexstring: string): Promise<bitcoinCash.rpc.FundRawResponse> {
