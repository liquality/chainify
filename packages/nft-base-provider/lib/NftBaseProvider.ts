...
export default abstract class NftBaseProvider extends Provider {
  ...
  constructor(contractABI: string, interfaceID: string) {
    super()
  ...
  private async _supportsInterface(contractInstance: Contract) {
    ...
    const txData = await contractInstance.populateTransaction.supportsInterface(this._interfaceID)
    const stateEncoded = await this.client.chain.call(txData.to, txData.data)
    const stateDecoded = contractInstance.interface.decodeFunctionResult('supportsInterface', stateEncoded)
