...
export default class EthereumScraperSwapFindProvider extends NodeProvider {
  ...
  async findAddressEvents (type, address, predicate, fromBlock, toBlock, limit = 250, sort = 'desc') {
    address = ensure0x(addressToString(address))

    for (let page = 1; ; page++) {
      const data = await this.nodeGet(`/events/${type}/${address}`, {
        limit,
        page,
        sort,
        fromBlock,
  ...
  async findClaimSwapTransaction (initiationTxHash, recipientAddress, refundAddress, secretHash, expiration, blockNumber) {
    ...
    const transaction = await this.findAddressEvents('swapClaim', initiationTransactionReceipt.contractAddress,
      tx => this.getMethod('doesTransactionMatchClaim')(tx, initiationTransactionReceipt))
    if (!transaction) return

    if (transaction._raw.status === true) return transaction
  }
  ...
  async findRefundSwapTransaction (initiationTxHash, recipientAddress, refundAddress, secretHash, expiration, blockNumber) {
    ...
    const transaction = await this.findAddressEvents('swapRefund', initiationTransactionReceipt.contractAddress,
