...
SwapScriptHashInput.prototype.getSignatures = function (
  transaction: any,
  privateKey: any,
  index: any,
  sigtype: any,
  ...
  if (bcUtil.buffer.equals(hashData, this.recipientPkh) || bcUtil.buffer.equals(hashData, this.refundPkh)) {
    ...
    const signat = (bcTransaction as any).Sighash.sign(
      transaction,
      privateKey,
      sigtype,
      index,
    ...
    return [
      new (bcTransaction as any).Signature({
...
SwapScriptHashInput.prototype.addSignature = function (transaction: any, signature: any, signingMethod: any) {
  ...
  const hashData = bcCrypto.Hash.sha256ripemd160(publicKey)
  if (!bcUtil.buffer.equals(hashData, this.recipientPkh) && !bcUtil.buffer.equals(hashData, this.refundPkh)) {
...
SwapScriptHashInput.prototype._createSignatures = function (signingMethod: any) {
  return bcDeps._.map(
    bcDeps._.filter(this.signatures, function (signature: any) {
      return !bcDeps._.isUndefined(signature)
    }),
...
SwapScriptHashInput.prototype.countSignatures = function () {
  return bcDeps._.reduce(
    this.signatures,
    function (sum: any, signature: any) {
...
function buildP2SHSwapIn(
  ...
  const s = bcScript.empty().add(signatures[0]).add(publicKey)

  if (secret) {
    s.add(secret).add(bcOpcode.OP_TRUE)
  } else {
    s.add(bcOpcode.OP_FALSE)
  }
  s.add(buildSwapOut(secretHash, recipientPkh, refundPkh, expiration).toBuffer())
  return s
}
...
function encodeExpiration(expiration: number) {
  ...
  const finalBuf: Buffer = bcUtil.buffer.concat([buf1, buf2])
