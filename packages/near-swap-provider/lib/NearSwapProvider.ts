import { SwapProvider, SwapParams, near } from "@liquality/types";
...
export default class NearSwapProvider
  extends Provider
  ...
  async initiateSwap(
    ...
    return this.getMethod("sendTransaction")(contractId, null, [
      ...
      transactions.transfer(
        new BigNumber(swapParams.value).toFixed().toString() as any
      ),
      transactions.deployContract(bytecode as any),
      transactions.functionCall(
        ABI.init.method,
        {
          secretHash: Buffer.from(swapParams.secretHash, "hex").toString(
            "base64"
          ),
  ...
  async verifyInitiateSwapTransaction(
    swapParams: SwapParams,
    initiationTxHash: string
  ): Promise<boolean> {
    const initiationTransaction = await this.getMethod("getTransactionReceipt")(
      initiationTxHash
    );
