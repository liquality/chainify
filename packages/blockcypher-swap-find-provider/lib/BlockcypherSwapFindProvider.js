...
export default class BlockcypherSwapFindProvider extends Provider {
  constructor (url) {
    super()
    this.url = url
  ...
  async findAddressTransaction (address, currentHeight, predicate) {
    ...
    const response = await this._axios.get(`/addrs/${address}/txs`)
...
BlockcypherSwapFindProvider.version = version
