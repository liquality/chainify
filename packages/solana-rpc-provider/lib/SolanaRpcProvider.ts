...
import { BigNumber, ChainProvider, Address, Block, Transaction, SendOptions, solana } from '@liquality/types'
...
export default class SolanaRpcProvider extends NodeProvider implements Partial<ChainProvider> {
  ...
  async getTransactionByHash(txHash: string): Promise<Transaction<any>> {
    const _txHash = '3Xu7GVdUrcx1wNXJJCGe7TuVB8RqSqkQA2ioReDZpGPyo6648otdZfaDRetpYjuK4MSizFF8469V7RDYwYDzdbDQ'

    const tx = await this.connection.getParsedConfirmedTransaction(_txHash)
