...
import { near } from "../../provider/node_modules/@liquality/types/dist/lib";
...
export default class NearRpcProvider extends NodeProvider {
  ...
  async getBlockByNumber(blockNumber: string, includeTx: boolean) {
  ...
  async generateBlock(numberOfBlocks: number) {
    return new Promise((resolve) =>
      setTimeout(resolve, numberOfBlocks * 10000)
    );
  }
  ...
  async _getBlockById(blockId: string, includeTx: boolean) {
    ...
    if (includeTx && !block.transactions && isArray(block.chunks)) {
      const chunks = await Promise.all(
        block.chunks.map((c: any) => this._rpc("chunk", c.chunk_hash))
      );
