...
export function shouldBehaveLikeLiqualityHTLCForEther(): void {
  beforeEach(async function () {
    const htlcFactory: LiqualityHTLC__factory = <LiqualityHTLC__factory>(
      await ethers.getContractFactory("LiqualityHTLC")
    );
...
function shouldInitiateEther(): void {
  ...
  it("should fail if sender doesn't match msg.sender", async function () {
    const blockTimestamp = (await ethers.provider.getBlock("latest")).timestamp + 1;
    await ethers.provider.send("evm_setNextBlockTimestamp", [blockTimestamp]);
    const htlcData = await getDefaultHtlcData(this.signers, blockTimestamp + 1);

    await expect(
      this.htlc.connect(this.signers.deployer).initiate(htlcData, { value: htlcData.amount }),
    ).to.be.revertedWith("LiqualityHTLC__InvalidSender");
  });
  ...
  it("should fail if amount doesn't match msg.value", async function () {
    ...
    await expect(this.htlc.connect(this.signers.sender).initiate(htlcData)).to.be.revertedWith(
      "LiqualityHTLC__InvalidMsgValue",
    );
  });
  ...
  it("should fail if expiration is smaller that block.timestamp", async function () {
    ...
    await expect(
      this.htlc.connect(this.signers.sender).initiate({ ...htlcData, expiration: blockTimestamp - 1 }),
    ).to.be.revertedWith("LiqualityHTLC__InvalidExpiration");
  });
...
function shouldClaimEther(): void {
  ...
  it("should fail if swap does not exists", async function () {
    ...
    const htlcData = await getDefaultHtlcData(this.signers, blockTimestamp + 60);
    await this.htlc.connect(this.signers.sender).initiate(htlcData, { value: htlcData.amount });
    await expect(this.htlc.claim(ethers.constants.HashZero, getDefaultSecret())).to.be.revertedWith(
      "LiqualityHTLC__WrongSecret",
    );
  });
